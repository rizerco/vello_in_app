...
cargo build --manifest-path rust/Cargo.toml --target ${TARGET} ${RELEASE_MODE}
...
if [ ! -d "swift/libs/${LIB_FOLDER}/" ]; then
  mkdir -p "swift/libs/${LIB_FOLDER}"
fi

cp rust/target/${TARGET}/${LIB_FOLDER}/libvello_in_app.a swift/libs/${LIB_FOLDER}/libvello_in_app.a
